...
# R3.0 - Interactive Administration Utility
# ==============================================================================
...
CONFIG_FILE="$SCRIPT_DIR/data/.mesh-dash_config"
# Legacy fallback for pre-R3.0 installs
if [ ! -f "$CONFIG_FILE" ] && [ -f "$SCRIPT_DIR/.mesh-dash_config" ]; then
...
# Docker venv path
if [ ! -f "$VENV_PYTHON" ] && [ -f "/opt/venv/bin/python3" ]; then
