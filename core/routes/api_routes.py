...
from core.auth import get_current_active_user, User, ensure_serializable
...
async def api_console(req: dict, user: User = Depends(get_current_active_user)):
...
async def monitor(req: dict, user: User = Depends(get_current_active_user)):
...
async def send_msg(req: dict, u: User = Depends(get_current_active_user)):
    from meshtastic_dashboard import (
